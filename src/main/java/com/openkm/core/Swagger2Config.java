...
public class Swagger2Config extends Swagger2Feature {
     ...
     * The reason why has been created the class rather using xml annotation into appContext.xml class is that the
     * number of properties available from there are limited and someones like licenseUrl raise and error
     ...
     * Issue with multiple jars server, to solve should be used setUsePathBasedConfig but at the end has not been
     * working and decided going into single server point rather than multiple for each service class
